...
import { UnlockBookModal } from './components/UnlockBookModal';
  ...
  useEffect(() => {
    const loadBook = async () => {
      ...
      try {
        ...
        setBook(data);
        if (data) {
          setIsUnlocked(bookService.isBookUnlocked(data.id));
        }
      } catch (error) {
  ...
  return (
    <div className="flex flex-col h-screen bg-stone-950 text-stone-50 overflow-hidden font-['DM_Sans',sans-serif]">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:block pl-3 border-l border-stone-800">
            ...
            <span className="text-[10px] text-stone-400">
              By Dr. Kunchala Hanumantha Rao • {isFreeToRead ? 'Complete Online Reading' : 'Premium Edition'}
            </span>
          </div>
        </div>
          ...
          <button
            ...
            className={`px-3 sm:px-4 py-1.5 rounded-full text-xs font-bold transition-all flex items-center gap-1.5 shadow-md cursor-pointer ${
              isFreeToRead 
                ...
                : 'bg-gradient-to-r from-amber-500 via-[#d4720a] to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white'
            }`}
          >
            <Download size={14} />
      ...
      <div className="flex-1 w-full relative bg-stone-950 overflow-hidden flex flex-col">
        {isFreeToRead ? (
          /* Free English PDF or Unlocked Telugu PDF Embedded Viewer */
            ...
            <div className="max-w-lg w-full bg-stone-900 border-2 border-amber-400 rounded-3xl p-6 sm:p-8 text-center space-y-6 shadow-2xl my-auto">
              ...
              <div className="space-y-2.5">
                ...
                <a
                  ...
                  className="w-full py-2.5 px-4 bg-stone-800 hover:bg-stone-700 text-stone-200 font-bold text-xs rounded-xl flex items-center justify-center gap-2 border border-white/10"
                >
                  <MessageCircle size={15} /> Buy on WhatsApp Assistance
                </a>
              </div>

              <p className="text-[11px] text-stone-400">
                Instant delivery of high-resolution PDF copy directly after 100% secure Cashfree payment.
              </p>
            </div>
          </div>
        )}
